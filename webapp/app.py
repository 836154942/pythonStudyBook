...
def init_jinja2(app, **kw):
    ...
    logging.info("set  jjiaja2 tempate path %s" % path)
...
async def logger_factory(app, handler):
    async def logger(request):
        logging.info("请求request %s ,  %s" % (request.method, request.path))
...
def data_timefilter(t):
    ...
    return u'%s 年%s 月 %s日 ' % (dt.year, dt.month, dt.day)
