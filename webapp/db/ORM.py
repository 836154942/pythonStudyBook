...
def log(sql, args=()):
    logging.info('要执行的SQL: %s    %s' % (sql, args))
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    def getStringValueOrDefault(self, key):
        return str(self.getValueOrDefault(key))
    ...
    def save(self):
        ...
        if rows != 1:
            logging.error('插入失败 failed to insert record: affected rows: %s' % rows)
        logging.info("插入成功")
